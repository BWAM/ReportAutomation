...
```{r}
#have to work on the grouping for the dissolved O2. 
#probably group by ID, and then one and the other.

sum.row<-selected.df %>% 
  subset(summarize_rows_operator %in% "&")

#use split apply combine to split it up by site?
temp<-split(sum.row,sum.row$parameter)#split into the 2 params, might be able to use the group to do the stats
temp<-split(temp,temp$dissolved_oxygen$statistic)

temp<-split(temp,temp$mean$dissolved_oxygen$block)#i think it's grouped ok now, but i can't figure out how to do the rest...
