...
contact<-UserP_all%>%
  dplyr::mutate(Primary=ifelse(UPFDH_PRIMARY_CONTACT=="A", yes=1, no=ifelse(UPFDH_PRIMARY_CONTACT=="B", yes=2, no=ifelse(UPFDH_PRIMARY_CONTACT=="C", yes=3, no=ifelse(UPFDH_PRIMARY_CONTACT=="D", yes=4, no=ifelse(UPFDH_PRIMARY_CONTACT=="E", yes=5, no=ifelse(UPFDH_PRIMARY_CONTACT=="F", yes=6, no=NA)))))),
  ...
  dplyr::select(SITE_ID, Primary, Secondary)%>%
  tidyr::pivot_longer(-SITE_ID, names_to = "Contact Type", values_to = "value")%>%
  order_sites(sites_table = Sites, sites_site_col = "SH_SITE_ID", df_site_col = "SITE_ID")



#second build plot
ggplot2::ggplot(contact, aes(x=SITE_ID, y=value, fill=`Contact Type`))+
