...
order_sites<-function(df, sites_table){
Sites2<-sites_table%>%
  rename(SITE_ID=SH_SITE_ID)

contact<-left_join(df, Sites2, by="SITE_ID")
contact<-contact%>%
  arrange(order)%>%
...
Variables<-df%>%
  ...
  order_sites(sites_table)%>%
  select(SITE_ID, `Water Clarity`, `Suspended Phytoplankton`, Periphyton, Macrophyte, Odor, Trash, `Discharge Pipes`)
