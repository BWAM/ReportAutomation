...
#now we need ph and temp for Nitrogen
tnp<-field %>% 
...
calcs.df<-calcs.df %>% 
  mutate(combo=paste(calcs.df$SITE_ID,calcs.df$COLL_DATE,sep = "_"))
  
calcs.df<-calcs.df %>% 
  aggregate(by=list(calcs.df$combo),first)
#this makes it all one line by the site/date combination of names

#now we have all theinformation in one dataframe, let's do the calcs.

#transform the units and value
calcs.df<-calcs.df %>% 
  mutate(corrected_value_2=case_when(
    result_unit %in% "mg/l"~result_value*1000,
    result_unit %in% "ug/l"~ result_value,
...
calcs.df<-calcs.df %>% 
  mutate(calc_value=case_when(
    ...
    #chemical_name %in% "nitrogen, ammonia (as n)"~
...
calcs.df$exceedance<-ifelse(calcs.df$corrected_value_2>calcs.df$calc_value,1,0)
...
bap.1<-bap %>% 
  select(SITE_ID,DATE,MMDH_BIO_ASMT_PROFILE_SCORE)


#also need BAP for this one,first merge the nnc (phos) with the baps
nnc.df<-merge(nnc,bap.1,by=c("SITE_ID","DATE"),all.x = TRUE)
...
nnc.final.df<-nnc.df %>% 
  mutate(exceed=case_when(
    SH_WQ_STANDARD %in% "A"& result_value>0.03 & MMDH_BIO_ASMT_PROFILE_SCORE<5 & chla>6 ~ 1,
    SH_WQ_STANDARD %in% "B"& result_value>0.03 & MMDH_BIO_ASMT_PROFILE_SCORE<5 ~1,
    SH_WQ_STANDARD %in% "C"& result_value>0.03 & MMDH_BIO_ASMT_PROFILE_SCORE<5 ~1,
    TRUE ~ 0
  ))
...
calcs_sum.df<-calcs.df %>% 
  group_by(SITE_ID,chemical_name) %>% 
  summarize(record_count=n(),mean=mean(corrected_value_2,na.rm =TRUE),num_exceed=sum(exceedance),min=min(corrected_value_2,na.rm =TRUE),max=max(corrected_value_2,na.rm =TRUE),median=median(corrected_value_2,na.rm =TRUE)) %>% 
  mutate_if(is.numeric, round, 3)
...
#summary for the BAP
bap.df<-bap %>% 
  select(SITE_ID,DATE,MMDH_BIO_ASMT_PROFILE_SCORE,year) %>% 
...
sites<-unique(chem$SITE_ID)
 ...
 for (i in seq_along(sites)) { 
  ...
  temp <-insitu_sum.df[insitu_sum.df$SITE_ID %in% sites[i], ]
   ...
   tab2<-set_caption(tab2,label)
   print(tab2)
   rm(temp)
   
  temp <-calcs_sum.df[calcs_sum.df$SITE_ID %in% sites[i], ]
   tab3<-table.f(temp, 2,4)
   label=paste("Metal Measurements:",sites[i])
   tab3<-set_caption(tab3,label)
   print(tab3)
   rm(temp)
  ...
  tab6<-set_caption(tab6,label)
  print(tab6)
  rm(temp)
  
  temp<-flow[flow$ID %in% sites[i], ]
  library(ggplot2)
  tab5<-ggplot(temp,aes(Date,Discharge..cfs.))+geom_point()
  print(tab5)
  rm(temp)
