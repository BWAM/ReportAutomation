...
chem_short.df<-chem.df %>% 
  select(SITE_ID,sample_date,SH_WQ_STANDARD,chemical_name,
         result_value,corrected_value,result_unit,threshold,t,direction,fraction,block,calculation,wqs_threshold,wqs_threshold_formula) %>% 
  rename("COLL_DATE"=sample_date)

#create exceedance column
chem_short.df$exceed<-NA

#need to do this only for the single vaues
chem_short.df<-chem_short.df %>% 
  mutate(exceed=case_when(
    block%in%"single"& direction=="<="& corrected_value>threshold~1,#this notation seems backwards in Zach's new stds file to me, but it probably makes sense to him. the direction is the direction <= means that the result should be less than or equal to the threshold.
    block%in%"single"&direction==">="& corrected_value<threshold~1,
    TRUE ~0
  ))
 
#now we need the ones that are not just single, but means by day? There are none, so it won't let me finish it, so I"m hashing it out.
#means_date<-chem_short.df %>% 
  #filter(block %in% "date") %>% 
  #group_by(c(SITE_ID,COLL_DATE)) %>% 
  #summarise(corrected_value_date=mean(corrected_value)) %>% 
  #mutate(exceed=case_when(
   # direction %in% "<="&corrected_value_date>threshold~1,
    #direction %in% ">="&corrected_value_date<threshold~1,
  #TRUE~0))
...
chem_calcs.df<-chem.df %>% 
  select(SITE_ID,sample_date,SH_WQ_STANDARD,chemical_name,result_value,corrected_value,result_unit,threshold,t,direction,wqs_threshold,wqs_threshold_formula,fraction,calculation,wqs_threshold,trout_class,type) %>% 
  rename("COLL_DATE"=sample_date) %>% 
  filter(wqs_threshold_formula!=""&!is.na(wqs_threshold_formula))
...
#now we need stayCALM for ammonia corrections
calcs.df<-calcs.df %>% 
  mutate(trout_water=ifelse((trout_class=="trout"|trout_class=="trout_spawning"),TRUE,FALSE)) %>% 
  filter(!is.na(ph))

calcs.df <- calcs.df %>%
  rowwise() %>% 
dplyr::mutate(threshold_calc = stayCALM::thresh_total_ammonia(.ph= ph,.temp= temperature,.trout_water= trout_water,.type= type))

calcs.df$exceedance<-if_else(calcs.df$corrected_value>calcs.df$threshold_calc,1,0)
...
chem_sum.df<-chem_short.df %>% 
  group_by(SITE_ID,chemical_name,fraction) %>% 
...
sites<-unique(site.loc$SH_SITE_ID)
 ...
 for (i in seq_along(sites)) { 
  ...
  temp <-calcs_sum.df[calcs_sum.df$SITE_ID %in% sites[i], ]
   ...
   label=paste("Dependent Measurements (Dissolved Metals and Ammonia):",sites[i])
