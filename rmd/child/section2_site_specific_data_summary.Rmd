...
calcs.df<-calcs.df %>% 
  aggregate(by=list(calcs.df$combo),min,na.rm=TRUE)
...
#probably doing the log of ALL the values, not row-wise

#transform the metal units to ug/L
...
nnc.df<-merge(nnc,bap,by=c("SITE_ID","DATE"))

#then merge with CLA for chlorophyll

nnc.df<-merge(nnc.df,cla,by=c("SITE_ID","DATE"))
...
nnc_sum.df<-nnc.final.df %>% 
  group_by(SITE_ID,chemical_name) %>% 
  summarize(record_count=n(),mean=mean(result_value),num_exceed=sum(exceed),min=min(result_value),max=max(result_value,median=median(result_value)))
