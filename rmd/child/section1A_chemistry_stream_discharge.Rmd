...
chem.statewide<-chem.statewide %>%
  ...
  summarise_at(vars(CHR_RESULT_VALUE),funs(mean(.,na.rm = TRUE),sd(.,na.rm = TRUE),n=n()))
...
chem.graph <- function(df, na.rm = TRUE, ...){
  ...
  for (i in seq_along(chem_list)) { 
    ...
    plot <- 
      ...
      xlab("Location ID")+
      geom_hline(yintercept=temp.statewide$ci95,color="firebrick")
...
insitu.statewide<-insitu.statewide %>%
  ...
  summarise_at(vars(RESULT),funs(mean(.,na.rm = TRUE),sd(.,na.rm = TRUE),n=n()))
...
theme(legend.position="right") + 
 ...
 scale_y_continuous(paste(df$PARAMETER_NAME[df$PARAMETER_NAME==chem_list.2[i]],df$RESULT[df$PARAMETER_NAME==chem_list.2[i]])) + 
 xlab("Location ID")+
   geom_hline(yintercept = temp.insitu.sw$ci95,color="firebrick")
