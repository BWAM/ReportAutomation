...
source(paste(file.path,"rmd_new_12_29","stayCALM_exceedance_calc.R",sep = "/"))
...
nnc_sum.df<-nnc_sum.df %>% 
  mutate(Parameter=case_when(
    Parameter %in% "PHOSPHORUS, TOTAL (AS P)"~"Total Phosphorus"
...
insitu_sum.df<-field %>% 
  ...
  summarise("Record Count"=n(),Mean=mean(RESULT,na.rm = TRUE),
...
chem_sum.df<-chem %>% group_by(SITE_ID,chemical_name,result_unit) %>% 
  summarise("Record Count"=n(),Mean=mean(result_value,na.rm = TRUE),
...
#change chemistry names
change<-read.csv(here::here("data","chem_names.csv"),stringsAsFactors = FALSE)

chem_sum.df<-merge(chem_sum.df,change,by.x="Parameter",by.y="chemical_name",all.x = TRUE)

chem_sum.df$Parameter<-NULL #taking out the old name
chem_sum.df<-chem_sum.df %>% 
  rename(Parameter=X)
...
#change insitu names
insitu_sum.df<-merge(insitu_sum.df,change,by.x="PARAMETER_NAME",by.y="chemical_name",all.x = TRUE)
insitu_sum.df$PARAMETER_NAME<-NULL
insitu_sum.df<-insitu_sum.df %>% 
  rename(Parameter=X,
         Site=SITE_ID) %>% 
  relocate(Parameter,.before=Site)

#rename flow axis
flow<-flow %>% 
  rename("Discharge"=Discharge..cfs.)
...
geom_point(data=df1,aes(x=SH_LONGITUDE,y=SH_LATITUDE),
                  color="yellow",size=4)+
...
sites<-unique(site.loc$SH_SITE_ID)
 ...
 for (i in seq_along(sites)) { 
  ...
  pwl_small<-unique(site.loc.small$SH_PWL_ID)
  
   
   cat("  \n\n###  ",sites[i], "| Waterbody Class:",stds_small,"| PWL_ID:",pwl_small)
  ...
  temp <-insitu_sum.df[insitu_sum.df$Site %in% sites[i], ]
  temp$Site<-NULL
   ...
   flextable::docx_value(tab3)}
     ...
     cat("  \n\n There were no water quality standard exceedances at this site during the sampling period.")
   }
   rm(temp)
     cat(rep("\n"))
   cat("<br><br>")
  ...
  label=paste("Biological Assessment Score Summary: ",sites[i])
  ...
  if (nrow(temp) > 0) {
    library(ggplot2)
    tab5<-ggplot(temp,aes(Date,Discharge)) +
      geom_point()+
        ylab("cfs")+
      labs(caption = "Discharge measured in cubic feet per second (cfs).")+
